...
const tradMethodButton = document.getElementById("trad-method-button");
const customModelButton = document.getElementById("model1-button");
const mobilenetButton = document.getElementById("mobilenet-button");
...
tradMethodButton.addEventListener("click", function () {
  socket.emit("start_method", { data: "start_trad" });
});

customModelButton.addEventListener("click", function () {
  socket.emit("start_method", { data: "start_model1" });
});
mobilenetButton.addEventListener("click", function () {
  socket.emit("start_method", { data: "start_mobilenet" });
});
stopButton.addEventListener("click", function () {
  socket.emit("start_method", { data: "stop_motion" });
});
      ...
      function sendData() {
        ...
        if (theta >= 0) {
          ...
          //speeds if in second quadrant
            ...
            var left_speed = Math.floor(
              (100 * (Math.PI - theta)) / (Math.PI / 2)
            );
        ...
        if (prev[0] != dir || prev[1] != x || prev[2] != y) {
          var data_s =
            dir +
            "," +
            String(Math.abs(left_speed)) +
            "," +
            String(Math.abs(right_speed)) +
            "\n";
